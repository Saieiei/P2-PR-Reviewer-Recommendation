...
🏷  Changes in this version (YYYY-MM-DD - Your Patch Date)
------------------------------------------------
• Set ChromaDB collection to use "cosine" distance for similarity.
• Existing similarity formula will now produce higher scores for similar items.
...
def build_or_update(): #
    ...
    # *** PATCHED LINE: Specify cosine distance metric ***
    col    = client.get_or_create_collection(
        name="gcbert_embeds",
        metadata={"hnsw:space": "cosine"}  # Use cosine distance
    )
    # Was: col    = client.get_or_create_collection("gcbert_embeds")  # default L2
...
def retrieve(diff: str, k: int, fname: str | None = None): #
    ...
    # For cosine space, "distances" from ChromaDB are 1 - cosine_similarity
...
def main(): #
    ...
    while True: #
        ...
        try:
            # dist_sq from retrieve() will be 1 - cosine_similarity
        ...
        for dist_sq, meta in res: #
            ...
            # dist_sq is 1 - cosine_similarity. For high actual similarity (e.g. 0.99), dist_sq is small (e.g. 0.01)
            # l2 will then be sqrt(dist_sq), e.g. sqrt(0.01) = 0.1
            l2   = math.sqrt(max(dist_sq, 0.0)) #
            # sim will be 1 / (1 + l2), e.g. 1 / (1 + 0.1) = 1 / 1.1 ~ 0.909 (high similarity)
            ...
            print(f"  Type       : {ttype}") #
            # dist_sq (1 - cosine_similarity) will be small for good matches
            print(f"  Distance   : {dist_sq:.4f}") #
            # sim (derived) will be high for good matches
