...
""" Finetuning CodeBERT for sequence classification on your PR-comment dataset. """
...
def train(args, train_dataset, model, tokenizer, optimizer):
    ...
    for idx, _ in enumerate(train_iterator):
        ...
        if args.do_eval and (
            ...
            results = evaluate(
                args, model, tokenizer, checkpoint=str(args.start_epoch + idx)
            )
            ...
            torch.save(
                ...
                os.path.join(checkpoint_last, "optimizer.pt"),
            )
            torch.save(
                scheduler.state_dict(),
                os.path.join(checkpoint_last, "scheduler.pt"),
            )
            with open(
                os.path.join(checkpoint_last, "idx_file.txt"),
                "w",
                encoding="utf-8",
            ) as idxf:
            ...
            if results.get("acc", 0) > getattr(train, "best_acc", 0):
                ...
                torch.save(
                    optimizer.state_dict(),
                    os.path.join(best_dir, "optimizer.pt"),
                )
                torch.save(
                    scheduler.state_dict(),
                    os.path.join(best_dir, "scheduler.pt"),
                )
    ...
    for eval_task, eval_output_dir in zip(
        ...
        if mode == "dev":
            eval_dataset = load_and_cache_examples(
                args, eval_task, tokenizer, ttype="dev"
            )
        else:
            eval_dataset, instances = load_and_cache_examples(
                args, eval_task, tokenizer, ttype="test"
            )
        ...
        if mode == "dev":
            with open(
                os.path.join(eval_output_dir, "eval_results.txt"),
                "a+",
            ) as writer:
...
def load_and_cache_examples(args, task, tokenizer, ttype="train"):
    ...
    try:
        ...
        if ttype == "test":
            examples, instances = processor.get_test_examples(
...
def main():
    ...
    ### Required parameters ###
    parser.add_argument(
        "--data_dir", type=str, required=True, help="Input data dir"
    )
    parser.add_argument(
        "--model_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Local path to pretrained CodeBERT",
    )
    parser.add_argument(
        "--task_name",
        type=str,
    ...
    parser.add_argument(
        "--num_train_epochs", default=3.0, type=float, help="Training epochs"
    )
    parser.add_argument(
        "--max_steps",
        default=-1,
        type=int,
        help="If > 0: total number of training steps",
    )
    parser.add_argument(
        "--warmup_steps", default=0, type=int, help="Linear warmup steps"
    )
    ...
    parser.add_argument(
        "--dev_file", default="dev.tsv", type=str, help="Dev file"
    )
    parser.add_argument(
        "--test_file", default="test.tsv", type=str, help="Test file"
    )
    parser.add_argument(
        "--pred_model_dir",
        default=None,
        type=str,
        help="Model dir for prediction",
    )
    parser.add_argument(
        "--test_result_dir",
    ...
    config = config_class.from_pretrained(
        ...
        num_labels=len(processors[args.task_name]().get_labels()),
