...
# --------------------------------------------------------------------------
# 9. Helper Function: Read Multiline Input
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# 10. Interactive Retrieval Loop.
# --------------------------------------------------------------------------
def main():
    while True:
        diff_input = read_multiline_input("Paste a diff snippet (or code). Type 'EOF' on a new line to finish input:")
        if not diff_input.strip():
            print("No input provided. Exiting.")
            break
        top_matches = retrieve_similar(diff_input, top_k=3)
        print("\n=== Top Matches ===")
        for rank, (score, item) in enumerate(top_matches, start=1):
            ...
            print("-" * 50)
        again = input("Would you like to test another diff snippet? (y/n): ").strip().lower()
        if again != "y":
            break
